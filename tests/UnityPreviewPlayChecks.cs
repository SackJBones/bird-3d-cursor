...
public sealed class UnityPreviewPlayChecks : MonoBehaviour
{
    ...
    private IEnumerator RunChecks()
    {
        ...
        visualObject.transform.localScale = Vector3.one * 0.1f;
        visualObject.GetComponent<Collider>().enabled = false;
        var visualRenderer = visualObject.GetComponent<Renderer>();
        var sharedVisualMaterial = Read<Material[]>("materials")[0];
        visualRenderer.sharedMaterial = sharedVisualMaterial;
        var originalBlock = new MaterialPropertyBlock();
        originalBlock.SetColor("_Color", Color.green);
        visualRenderer.SetPropertyBlock(originalBlock);
        var feedback = target.AddComponent<BirdInteractableVisual>();
        feedback.source = interactable;
        feedback.visual = visualRenderer;
        feedback.duration = 0.03f;
        target.SetActive(true);
        for (int i = 0; i < 4; i++) yield return null;
        Check(selections == 0 && deselections == 0, "Idle interactable must not emit deselection");
        yield return new WaitForSecondsRealtime(0.08f);
        Check(interactable.IsHovered && !interactable.IsSelected, "Read-only state must expose hover independently of selection");
        Check(Mathf.Abs(visualObject.transform.localScale.x - 0.1f * feedback.hoverScale) < 0.0001f, "Hover feedback must finish at its target scale");
        var feedbackBlock = new MaterialPropertyBlock();
        visualRenderer.GetPropertyBlock(feedbackBlock);
        Check(feedbackBlock.GetColor("_Color") == feedback.hoverColor, "Hover feedback must apply its configured color");
        Check(sharedVisualMaterial.color == Color.cyan, "Feedback must not mutate the shared material");
        Set("pressed", true);
        for (int i = 0; i < 4; i++) yield return null;
        Check(selections == 1 && deselections == 0, "Held event-only drag must select once");
        yield return new WaitForSecondsRealtime(0.08f);
        Check(interactable.IsSelected && Mathf.Abs(visualObject.transform.localScale.x - 0.1f * feedback.selectedScale) < 0.0001f,
            "Selected feedback must take precedence over hover");
        visualRenderer.GetPropertyBlock(feedbackBlock);
        Check(feedbackBlock.GetColor("_Color") == feedback.selectedColor, "Selection feedback must finish at its configured color");
        if (renderPreview) CaptureCamera(camera, cursors, "preview-feedback.png");
        Set("pressed", false);
        for (int i = 0; i < 4; i++) yield return null;
        ...
        yield return new WaitForSecondsRealtime(0.08f);
        Check(Mathf.Abs(visualObject.transform.localScale.x - 0.1f) < 0.0001f, "Disabled interaction must return visuals to idle");
