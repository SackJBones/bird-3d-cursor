...
Temporal recording audit: UnityQuestTemporalChecks.cs lives in the generated hands project's Assets/Editor folder (copied by the build runner). Run Unity 2022.3.22f1 with -batchmode -nographics -buildTarget Android -projectPath <generated-project> -executeMethod UnityQuestTemporalChecks.Run -birdJointTrace <private-file.jsonl> -logFile <log>. Omit -quit; it exits itself. Inspect joint-temporal-result.txt and private JointTemporalChecks/near-returns.csv. Separate hand state, independent double filter references and observed outputs after a known fist reset check numerical behavior. Near-return metrics deliberately report MEASURED: reference correctness is not a usability pass. This does not build/deploy, and is separate from optional unsmoothed geometric replay in build preflight. See docs/modernization/HAND-RETURN-FILTER.md for the measured unresolved far-history return problem. Keep recordings and per-event results outside Assets and public Git.
