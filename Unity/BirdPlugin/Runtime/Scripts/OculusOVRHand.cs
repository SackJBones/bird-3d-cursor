...
namespace Bird3DCursor
{
    ...
    public class OculusOVRHand : Hand
    {
        ...
        public OculusOVRHand(Hand.Chirality chirality) : base(chirality)
        {
            
            // Find all game objects with an OVRHand component
            OVRHand[] handComponents = GameObject.FindObjectsOfType<OVRHand>();
            foreach (var handComponent in handComponents)
            {
                OVRSkeleton skel = handComponent.gameObject.GetComponent<OVRSkeleton>();
                if ((skel.GetSkeletonType() == OVRSkeleton.SkeletonType.HandLeft && chirality == Hand.Chirality.Left) ||
                    (skel.GetSkeletonType() == OVRSkeleton.SkeletonType.HandRight && chirality == Hand.Chirality.Right))
                {
                    ovrHand = handComponent;
                    skeleton = skel;
                    break;
                }
            }
