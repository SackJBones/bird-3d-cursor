...
Latest integration checkpoint: a fresh local UPM installation passes 63 checks in Unity 2020.3.33f1, with separate runtime/editor assemblies and no mandatory hand SDK. A Windows x64 Mono player also builds and runs a startup smoke test with the runtime assembly included and editor assembly excluded. Prioritize optional adapter assembly boundaries/registration next. See the package README for unvalidated backend and legacy-GUID migration limits. This supersedes the earlier inventory's missing-asmdef/mandatory-Ultraleap findings.
