...
## Avatar event routing, 2026-09-25 13:15 UTC

UnityAvatarInputChecks.RunAvatarEvents passed in actual ClientSim/compiled Udon. Creating a remote simulator player through ClientSimMain exercised its join path without disturbing either calibrated local cursor. Sending a remote avatar-change event through the SDK's ClientSimUdonManagerEventSender also preserved calibration. Sending the local event immediately cleared both adapters' dataReady/calibration and hid/cancelled their cursors. With requireCalibration enabled, subsequent valid samples kept both hidden; explicit recalibration recovered raw targets of 0.3 m within 2 mm. Clicks stayed disabled, and the temporary remote player was removed with the original player count restored.

SDK 3.10.5 ClientSimPlayerHeightManager.OnPlayerJoined dispatches _onAvatarChanged through that same SDK event sender. No avatar-replacement implementation was found in ClientSim source. The local event in this check is deliberately replayed, not caused by replacing an avatar asset. Actual avatar switching and physical/networked validation remain open; this result establishes callback routing and handler behavior only.
