...
- [ ] Adapt the menu and spherical interaction into supported VRChat Udon components/actions, with compiled VM and ClientSim tests. Do not assume the standalone MonoBehaviours run inside VRChat.
...
- [x] Exercise rapid return and mixed-depth trail geometry with deterministic 30/72/120 Hz renderer updates; fix and regress the detached live trail tip. Hardware frame pacing and subjective motion remain unverified.
- [x] Reduce measured outward-size lag bias across update rates using target integration; verify analytic constant/exponential responses, irregular intervals, linear-ramp error and immediate return sizing.
- [x] Measure shared-shell stereo error and opaque-world occlusion with real Unity renders and direct-world controls; preserve the two known occlusion failures separately from passing controls.
- [ ] Implement and validate logical-depth-aware presentation for opaque/partial occlusion and trails, with an explicit optional locator policy; preserve the logical point and near physical size.
...
## Rapid return / trail-tip regression - 2026-09-25, 19:23 UTC cycle

Added UnityDepthMotionChecks.Run, an actual Unity Play Mode renderer/mesh fixture with explicit simulated timestamps at 30/72/120 Hz. It exercises all three size modes over 1341 updates: outward motion from 0.4 m to one million meters, a 0.12 s return, a one-update billion-meter jump/return, moving-eye history reprojection, selection changes and clear/recovery. It inspects the real generated sphere and ribbon mesh rather than a duplicate renderer. This is deterministic time stepping within a fixture, not measured device frame pacing, an animated playback evaluation or subjective perception validation.

The initial size/history checks passed, and 18 real Unity captures were produced. Visual inspection exposed a small detached trail tip in the 72 Hz outward stroke. A targeted endpoint assertion then failed against the previous renderer at 72 Hz, update 1. Root cause: the 14 ms history sampling gate skips updates at 72/120 Hz while the cursor moves every update. Fixed BirdDepthVisual to refresh its latest sample on intervening updates, retaining a separate sampling clock so new history still accumulates. The buffer remains 96 points; geometry, interaction, range/filter and near-size policies are unchanged.

Final motion regression PASS: 1341 updates, 1011 with both near (<4 m) and far (>50 m rendered depth) history, 190087 assertions, max near cursor diameter 0.032000 m, max near trail width 0.002000 m, max display radius 500.000 m. Every update checks live endpoint agreement within 0.3 mm, and every mode/rate must accumulate multiple history samples. Recovery cannot connect to stale far history. Captured and inspected the corrected far endpoint and rapid-return snapshots; no giant near marker/trail appeared. Generated PNGs, raw CSV and logs remain in ignored Validation/QuestHands2022/DepthMotionCaptures. The fixture report wording was clarified afterward to say simulated renderer updates; numerical checks did not change.

At 72 Hz on the chosen outward stroke, the lagged core diameter at t=0.597222 s is 4.85378 m versus 21.6590 m without lag at logical distance 143161.8 m. At t=1.083333 s during return, both are already 0.343730 m at distance 36.0565 m, and both return to 0.032 m at 0.4 m by t=1.125 s. These are experiment measurements, not evidence of better perceived depth. Stereo/occlusion studies, actual frame pacing/performance and user feel remain pending. No new Udon/VRChat claim is made; the core math was not changed.
Measured growth-rate sensitivity remains: at t=0.5 s of the extreme outward sweep (logical distance 3988.16 m), lagged diameter was 0.95148/0.81119/0.77278 m at 30/72/120 Hz respectively. Near-size and return checks pass, but equal growth feel across rates is not established. Consider integrating the changing target across samples in a later presentation-only cycle; retain immediate inward sizing.

The full v0.3 ARM64 OpenXR APK build passed in Unity 2022.3.22f1, including the 400-sample parity, 2402-sample palm sweep and 91 depth-math preflight checks. Artifact is 19,963,112 bytes; SHA256 4C126AE2BDFE82A25B8217E3B6FDDA6A7D49EBE0AAEB6542EC9A841D959FB195. The Quest was available for one bounded deployment: adb install -r returned Success, installed package org.bird3d.livehands reports versionName 0.3 / arm64-v8a, and Android accepted the launch intent. No physical headset interaction was requested. Heavy authored project unchanged; do not make a no-op heavy commit.
After one resume, v0.3 startup logs confirm xr=True and subsystem=True, with Dynamic samples increasing approximately 72/s. Both hands remain 0/18 while unattended; this is startup evidence only. The physical v0.3 view and feel are deferred.

## Integrated outward size growth - 2026-09-25, 21:23 UTC cycle

Completed one presentation-only checkpoint from the prior measured lag sensitivity. A new real-renderer regression reproduced the v0.3 bias: 23.124% size spread at the same t=0.5 s outbound pose across simulated 30/72/120 Hz. The previous one-pole step used the current target for the whole interval. BirdDepthVisual now remembers the previous target and integrates a log-linear target exactly over the growth interval. It still accepts a constant-target step when no previous sample is supplied. Clear/recovery and mode changes reset target history. Decreasing targets cannot cause late catch-up growth; shrinking to a smaller target and restoration of fixed near size remain immediate. No Bird fit, polynomial, Kalman, interaction or Udon code changed.

PASS: real Unity motion fixture, 1341 deterministic renderer updates / 1011 mixed-depth updates / 190088 assertions, with the same 32 mm near diameter, <=2 mm near trail width, <=500 m display radius, attached tips and clear/recovery invariants. The outbound diameters at logical distance 3988.16 m are now 0.716249 / 0.716673 / 0.716778 m for 30/72/120 Hz, reducing spread to 0.074%. Eighteen Unity snapshots were rendered; representative far/outbound and near/return images were inspected. These are simulated timestamps, not measured hardware frame pacing or a perceptual test.

PASS in the Unity build preflight: 115 visual math checks. New checks compare exponential growth at multiple slopes and 30/72/120/240 Hz against the independent closed-form continuous solution within 0.002% relative error, and cover irregular intervals, held targets, a linear-ramp approximation within 0.2%, decreasing targets and zero elapsed time. Exactness applies to the assumed exponential segments, not arbitrary unseen hand motion. The original 400-sample parity and 2402-sample palm continuation preflights remain passing. DEPTH-GROWTH.md records the model and before/after measurements. Next prioritize stereo/parallax and occlusion experiments or independent world/lifecycle work, keeping actual perceived feel and device timing deferred. Do not claim the visual problem solved from these numerical checks.
Full v0.4 build PASS: Unity 2022.3.22f1 / ARM64 OpenXR, 19,963,384-byte APK, SHA256 B95B426B42A9109A9E18D75450010BE4E4570818168930D1FFB49C734F979138. The available Quest accepted adb install -r (Success); package inspection confirms org.bird3d.livehands versionName 0.4 / arm64-v8a, and Android accepted the launch intent. Physical view/feel remains deferred. Heavy authored repository is unchanged; no no-op commit is needed there.
The bounded v0.4 startup check confirms xr=True / subsystem=True and Dynamic samples increasing about 72/s. Both hands are 0/18 while unattended. This confirms runtime startup, not physical tracking quality or visual feel.

## Stereo / logical occlusion measurement - 2026-09-26, 01:25 UTC cycle

Completed a bounded diagnostic checkpoint with actual Unity 2022.3.22f1 rendering. Added UnityDepthStereoChecks.Run to the generated-project helpers. It uses parallel mono cameras, a 1024-square/60-degree/4x-MSAA target, 0.005 m near clip and 2000 m far clip, testing eight logical distances (0.4 m through one billion m) at 58/64/72 mm eye spacing. This measures shared-shell binocular geometry, not OpenXR single-pass rendering or physical headset comfort. The runtime presentation/core and installed v0.4 APK remain unchanged; no headset polling, build or deployment was needed.

MEASURED: 24 stereo configurations, maximum excess disparity 0.127686 pixel in this setup; no added disparity through the unprojected 100 m volume. Sixteen real eye images at 64 mm eye spacing passed visibility and centroid controls within 0.6 pixel raster tolerance. This bound is not a perceptual comfort threshold. Far actual disparity also approaches floating-point/raster resolution, so the recorded matrix differences are not claimed to be arbitrarily precise binocular measurements.

CONFIRMED LIMITATION: the projected cursor is visible through an opaque 600 m wall when its logical point is at 1000 m or one million m (8 cyan pixels survive in each case). The 1000 m point renders at 376.923 m, explaining the wrong depth ordering. Near 2 m/1 m wall, far 1000 m/300 m wall and 1000 m/1500 m wall controls behave as expected. An independent ordinary Unity sphere at its actual 1000 m position disappears behind the 600 m wall and reappears when that wall is disabled. Five control checks pass, but two of five logical-visibility cases remain wrong. The probe deliberately reports MEASURED and the mismatch count, not an occlusion PASS. Twenty-three images were captured; the wrong projected-wall case, correct direct-world-wall case and a near stereo image were inspected.

Preserved the small stereo/occlusion CSVs under docs/modernization/measurements; bulky PNGs/logs remain ignored in Validation/QuestHands2022/DepthStereoCaptures. DEPTH-STEREO-OCCLUSION.md records the setup, results and next boundary. Next prototype logical-depth-aware rendering while retaining far clip visibility, and distinguish an intentional locator overlay from a solid world-occluded point. Check partial occlusion and trails, not only a center ray; never clamp Bird's logical point or alter click policy merely to hide a visual. Preserve physical near size and verify stereo plus the Android graphics path before replacing the experimental shell. World occlusion and headset perception remain unchecked in PLAN.md. Heavy authored project unchanged; no no-op commit there.
