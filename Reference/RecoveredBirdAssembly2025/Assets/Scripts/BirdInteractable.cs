...
public class BirdInteractable : MonoBehaviour
{
    ...
    Rigidbody thisRigidbody;//same but rigidbody if any
    ...
    private bool previousUseGravity;//for objects with rigidbodies, whether the object had gravity enabled or not
    ...
    void decideUsingPhysics() {
        usingPhysics = thisRigidbody != null;
    }
    ...
    void doUpdate()
    {
        ...
        else if (activateType == ActivateType.Touch)
        {
            ...
            if (selectingMe)
            {
                doSelect(!alreadySelectingMe);
            }
            else
            {
                doDeselect(alreadySelectingMe);
            }
        }
        Vector3 targetPos;
        ...
        if (following)
        {
            Vector3 newPosition = filter.Update(targetPos);// actually decide where to move the object. Uses Kalman filter defined above.
            if (usingPhysics) {
            ...
            Quaternion newRotation = transform.rotation;//by default, keep this rotation
            ...
            else if (orientationMode == OrientationMode.Horizontal)
            {
                ...
                newRotation = Quaternion.Euler(Vector3.Scale(lookAtRotation, Vector3.up));
            }
            else if (orientationMode == OrientationMode.TwoAxis)
            {
                newRotation = Quaternion.LookRotation(directionVector);
            }
            ...
            {
                newRotation = sourceRotation * Quaternion.AngleAxis(90, Vector3.up);
            }
            ...
            {
                newRotation = sourceRotation * Quaternion.AngleAxis(90, Vector3.forward);
            }
            ...
            {
                newRotation = sourceRotation * Quaternion.AngleAxis(90, Vector3.right);
            }
    ...
    void doSelect(bool invoke=true) {
        following = true;
        if (usingPhysics) {
            previousUseGravity = thisRigidbody.useGravity;
    ...
    void doDeselect(bool invoke=true) {
        following = false;
        if (usingPhysics) {
            thisRigidbody.useGravity = previousUseGravity;
        }
        if (invoke)
            OnSelect.Invoke();
    }
