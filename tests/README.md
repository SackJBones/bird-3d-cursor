...
Palm/depth comparison (v0.2): UnityPalmFitChecks is shared by UnityQuestHandsChecks and UnityUdonSphereFitChecks. Copy it to Assets/Editor in the standalone hands project, or Assets/BirdGenerated/Runtime in BirdWorld alongside the compiled-VM helper. Restore the production fitter source/meta before the Udon run. The fixture covers 1601 near-flat/inverted curvature samples and 801 long-reach samples plus mirrored/rigid/legacy/invalid/recovery cases. The Quest build runner copies it automatically. BirdSphereFit's palm continuation is opt-in; the avatar adapter does not enable it.
