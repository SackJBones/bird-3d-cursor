...
- [ ] Resolve the automatic-disable lifecycle check, then validate missing-bone, scale and avatar-change behavior under ClientSim. Prove two-hand cursor/click behavior with real hardware before making a polished world. Provide a clearly labeled controller/desktop fallback where true joint input is unavailable.
